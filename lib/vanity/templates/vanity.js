...
Vanity.tooltip = function(event, pos, item) {
  if (item) {
    if (this.previousPoint != item.datapoint) {
      ...
      jQuery('<div id="tooltip">' + dt.getUTCFullYear() + '-' + (dt.getUTCMonth() + 1) + '-' + dt.getUTCDate() + "<br>" + y + '</div>').css( {
...
Vanity.metric = function(id) {      
  var metric = {};
  metric.chart = jQuery("#metric_" + id + " .chart");
  ...
  metric.plot = function(lines) {
    jQuery.each(lines, function(i, line) {
      jQuery.each(line.data, function(i, pair) { pair[0] = Date.parse(pair[0]) })
    });
    var plot = jQuery.plot(metric.chart, lines, metric.options);
...
jQuery(function() {
  var checkboxes = jQuery("#milestones input:checkbox");
  checkboxes.bind("change", function() {
    var markings = [];
    checkboxes.filter(":checked").each(function(i, checkbox) {
      var start = Date.parse(jQuery(this).attr("data-start"));
      var end = Date.parse(jQuery(this).attr("data-end"));
    ...
    jQuery(".metric .marking.label").remove();
    jQuery(".metric .chart").each(function() {
      ...
      jQuery.each(markings, function(i, mark) {
        if (mark.xaxis && mark.xaxis.from && mark.label) {
          ...
          jQuery('<div class="marking label"></div>').css({left:o.left+4}).text(mark.label).appendTo(chart);
        }
      });
    });
  });

  jQuery(".experiment.ab_test a.button.chooses").live("click", function() {
    var link = jQuery(this);
    jQuery.ajax({
      data: 'authenticity_token=' + encodeURIComponent(document.auth_token),
      success: function(request){ jQuery('#experiment_' + link.attr("data-id")).html(request) },
